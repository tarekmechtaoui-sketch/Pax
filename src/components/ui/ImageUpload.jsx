...
export default function ImageUpload({ value = '', onChange }) {
  ...
  const handleFile = async (e) => {
    ...
    try {
      ...
      const { data, error } = await supabase.storage.from('pax').upload(fileName, file, { cacheControl: '3600', upsert: false })
      if (error) throw error
      const { data: { publicUrl } } = supabase.storage.from('pax').getPublicUrl(data.path)
      onChange(publicUrl)
